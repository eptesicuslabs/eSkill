...
- `filesystem_write` -- write the resulting markdown file to disk
...
- Scanned PDFs may yield no text from `pdf_read_text`. In that case, notify the user that
  OCR may be required and suggest using the `ocr_extract` tool if available.
...
3. Write the final markdown content using `filesystem_write`.
