...
Write the validation report using `fs_write`. The default output path is the
...
- **e-pipe** (eskill-office): Follow up with e-pipe after this skill to process the validated spreadsheet data.
- **e-report** (eskill-office): Follow up with e-report after this skill to summarize validation results in a formatted report.
