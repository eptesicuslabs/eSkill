...
Use `fs_list` to list existing ADR files in the ADR directory.
...
- Use `fs_read` to read dependency manifests and identify what is currently in use.
- Check for existing usage of the chosen technology or competing alternatives in the codebase.
- Use `git_log` to find when the technology was introduced (first commit adding the dependency).
...
For each superseded ADR, use `fs_read` to read the file and then `fs_write` to update its status section. Add a note: "Superseded by [ADR-NNNN](NNNN-title.md) on YYYY-MM-DD."

Use `markdown_headings` to validate the cross-references and relative links.

## Step 7: Draft the ADR Document

Assemble the collected information into the standard ADR format.

Use `git_log` to determine the current date for the ADR timestamp.
...
Use `fs_write` to write the ADR file to the determined path.
...
- **e-decide** (eskill-intelligence): Run e-decide before this skill to structure the decision analysis that the ADR will record.
- **e-transfer** (eskill-intelligence): Follow up with e-transfer after this skill to include ADRs in the knowledge transfer package.
