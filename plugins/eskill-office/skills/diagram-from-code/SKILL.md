...
- `filesystem_list_dir` -- list directory contents to discover project structure
- `filesystem_tree` -- display a directory tree for high-level overview
...
- `filesystem_write` -- write diagram source files to disk
...
1. Call `filesystem_tree` on the project root to get an overview. Limit depth to 3 or 4
...
4. Note configuration files, test directories, and build artifacts so they can be excluded
   from the diagram.
5. Call `filesystem_list_dir` on key directories to enumerate source files.
