...
Use `filesystem` tools (list_dir, tree, read_text) to survey the project structure. Determine:
...
Use `filesystem` tools (write_text, edit_text) to create or modify files. Write each component to its own file following the project's file organization convention.
...
Use `responsive-layout-check` from eskill-frontend to verify the responsive implementation.

### Step 8: Validate Output

Before reporting completion, verify the implementation:

1. **Token compliance**: Run `design-system-audit` from eskill-frontend to confirm no hardcoded values bypass the design system.
2. **Accessibility**: Run `accessibility-scan` from eskill-quality to verify WCAG compliance.
3. **CSS quality**: Run `css-optimization` from eskill-frontend to catch redundancy or specificity issues.
4. **File integrity**: Use `filesystem` tools (list_dir) to confirm all generated files are in the correct locations.
...
- **component-scaffold** (eskill-frontend): Use component-scaffold for generating boilerplate component files. Use frontend-design when the visual design and interaction quality of the component matters.
- **design-system-audit** (eskill-frontend): Run design-system-audit after frontend-design to verify token compliance.
- **responsive-layout-check** (eskill-frontend): Run responsive-layout-check after frontend-design to verify responsive behavior.
- **css-optimization** (eskill-frontend): Run css-optimization after frontend-design to clean up generated CSS.
- **accessibility-scan** (eskill-quality): Run accessibility-scan after frontend-design to verify WCAG compliance.
- **storybook-scaffold** (eskill-frontend): Follow up with storybook-scaffold to generate Storybook stories for new components.
