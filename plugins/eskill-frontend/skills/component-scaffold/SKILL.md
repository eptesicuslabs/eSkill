...
Use `filesystem` tools (read_file) to read `package.json`. Check the `dependencies` and `devDependencies` fields against this table:
...
Scan the project to determine how styles are authored. Use `filesystem` tools (list_directory) on the component directories and check for these patterns:
...
Read two or three representative component files using `filesystem` tools (read_file) and extract these conventions:
...
Use the test framework detection logic from the test-scaffolding skill (check `package.json` for vitest, jest, or the framework's testing library).
...
After generating all files, confirm the directory structure is consistent with existing components. Use `filesystem` tools (list_directory) to compare the new component directory layout against an existing component directory.
...
- **storybook-scaffold** (eskill-frontend): Follow up with storybook-scaffold after this skill to generate Storybook stories for the new components.
- **design-system-audit** (eskill-frontend): Run design-system-audit before this skill to ensure new components align with the design system.
