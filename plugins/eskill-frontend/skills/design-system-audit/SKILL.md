...
Use `filesystem` tools (list_directory, read_file) to search for design token sources. Check these locations in order:
...
Use `ast_search` from the eMCP AST server to find string literals in style-related code. Additionally, use `run_command` from the eMCP shell server to run targeted searches:
...
- **css-optimization** (eskill-frontend): Follow up with css-optimization after this skill to clean up CSS inconsistencies found during the audit.
- **accessibility-scan** (eskill-quality): Run accessibility-scan alongside this skill to verify design system components meet accessibility standards.
