...
- If information gaps are significant, consider running the research-workflow skill for specific options before continuing with evaluation.
...
### Step 8: Store the Decision in Memory

Persist the decision and its rationale in the eMCP memory graph.

- Create an entity using `create_entities` with type "decision":
  - Name: a slug version of the decision title (e.g., "message-queue-selection").
  - Include the decision statement, date, and status (proposed, accepted, deprecated, superseded).
...
- Create relations using `create_relations`:
  ...
  - "evaluated": the decision entity evaluated each option entity.
- If option entities already exist in memory (e.g., from a previous research-workflow run), link to them rather than creating duplicates.
