...
Persist discoveries using the eMCP docs and reasoning servers as the exploration progresses.

**Index findings in docs server**:
- After each depth level, use `docs_index` to index any documentation files read so their contents become searchable via `docs_search`.

**Build reasoning chains**:
- Call `think_start` to create a reasoning chain for the learning exploration, with the learning goal as the chain's goal.
- After each depth level, call `think_step` with type="observation" for newly discovered components, services, and concepts. Include the exploration level for context (e.g., "Level 2: This module exports three public functions for user management.").
- Call `think_step` with type="thought" and evidenceFor/evidenceAgainst links as dependencies and connections are discovered.
- Use consistent naming across levels so that deeper exploration enriches existing reasoning chains rather than creating redundant observations.
- Before recording a new concept, use `docs_search` to check if similar content already exists, and use `think_replay` to review prior observations from a previous session or skill run.
...
- When encountering unfamiliar technologies or patterns, note them for separate study rather than derailing the current exploration. The e-research skill can be used for deep dives into specific technologies.

## Edge Cases

- **Codebases with no tests or documentation**: Without tests-as-specs or docs, the learning path relies entirely on code reading. Prioritize entry points and high-connectivity modules as starting points for exploration.
- **Rapidly changing codebases**: If many files were modified in the last week, the learning path may describe code that has already changed. Note the exploration date and recommend re-running for actively developed areas.
- **Extremely large codebases (10K+ files)**: Full exploration is impractical. Focus learning on a specific subsystem or feature area rather than attempting to map the entire codebase.
- **Unfamiliar language or framework**: When the learner does not know the project's primary language, interleave language learning resources with codebase exploration. Do not assume language proficiency.
- **Legacy code with misleading names**: Old codebases may have functions named for behaviors they no longer perform. Cross-reference names with actual implementation before recording concepts in the learning journal.

## Related Skills

- **e-carto** (eskill-intelligence): Run e-carto before this skill to map the codebase structure that learning paths will traverse.
- **e-graph** (eskill-intelligence): Run e-graph before this skill to provide the concept relationships that inform learning sequences.
