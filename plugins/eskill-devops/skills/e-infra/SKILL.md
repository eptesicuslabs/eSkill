...
- Infrastructure-as-code files in the project (Terraform, Docker Compose, Kubernetes manifests, CloudFormation, or Pulumi).
- The eMCP filesystem and data_file_read servers available for parsing IaC configs.
- Knowledge of the target cloud provider(s) for provider-specific security checks.
...
Read the content of each identified IaC file using `fs_read` from the filesystem tool.
...
- **Terraform modules with remote sources**: Modules sourced from git, registries, or S3 cannot be inspected without fetching them first. Flag external module references and recommend running `terraform init` before review.
- **Helm chart values vs. template defaults**: Kubernetes resources generated by Helm have defaults in Chart templates that may differ from values.yaml overrides. Review both the template defaults and the values files.
- **Docker Compose extends and overrides**: Services using `extends` or override files (`docker-compose.override.yml`) may inherit insecure settings from the base file. Trace the full merge chain.
- **CloudFormation conditions and mappings**: Resources conditionally created based on parameters or mappings may have different security postures per environment. Evaluate all condition branches.
- **Multi-cloud configurations**: Projects deploying to multiple cloud providers may have inconsistent security baselines (e.g., encryption at rest enabled on AWS but not GCP). Audit each provider independently.

## Related Skills

- **e-config** (eskill-quality): Run e-config alongside this skill to verify application-level configs match infrastructure expectations.
- **e-scan** (eskill-quality): Run e-scan alongside this skill to identify security misconfigurations in infrastructure code.
- **e-terra** (eskill-devops): Run e-terra alongside this skill when the infrastructure uses Terraform modules.
