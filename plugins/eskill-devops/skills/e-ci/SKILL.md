...
- A project with a detectable build system (package.json, pyproject.toml, Cargo.toml, go.mod, or equivalent).
- The eMCP filesystem and data_file_read servers available for project introspection.
- Knowledge of the target CI platform (GitHub Actions, GitLab CI, CircleCI, etc.) or willingness to accept the auto-detected default.
...
- **Monorepo with multiple languages**: A repo containing both a Node.js frontend and a Python backend needs separate build/test jobs with proper path filters so changes in one area do not trigger the other's pipeline.
- **Private package registries**: Projects pulling from private npm, PyPI, or Maven registries need authentication tokens configured as CI secrets, which the generated config must reference without hardcoding.
- **Platform-specific tests**: Projects with OS-specific code (Windows services, macOS GUI, Linux kernel modules) need matrix builds that include the relevant runner OS, not just the default ubuntu-latest.
- **Long-running test suites**: Test suites exceeding 30 minutes need parallelization or test splitting configured in CI to avoid timeout failures and slow feedback loops.
- **Self-hosted runners**: Organizations using self-hosted CI runners may lack pre-installed tools. The generated config should include tool installation steps or validate runner capabilities.

## Related Skills

- **e-init** (eskill-meta): Run e-init before this skill to establish the project structure that CI pipelines will build and test.
- **e-deploy** (eskill-devops): Follow up with e-deploy after this skill to define the deployment steps the CI pipeline will execute.
