...
- A project with a runnable test suite and build system.
- The eMCP filesystem, shell, data_file_read, and test_run servers available.
- Access to environment configuration files for the target deployment environment.
...
- Use `sys_notify` from the eMCP notify tool to send the checklist results.
...
- **Detached HEAD state**: CI systems often check out specific commits in detached HEAD mode. Git state checks must handle this without reporting false failures.
- **Lockfile out of sync**: The lockfile may be committed but diverge from the manifest (package.json changed without running npm install). Flag this as a blocking issue since it causes non-reproducible builds.
- **Pending database migrations**: Code changes may depend on schema migrations that have not been applied to the target environment. Check for unapplied migrations when the project uses an ORM.
- **Environment variable placeholders**: Config files may contain `${VAR}` placeholders that are resolved at deploy time. Validate that all referenced variables exist in the target environment's secret store.
- **Multi-stage deployments**: When deploying to staging before production, the checklist should run against the staging configuration first rather than assuming a single target environment.

## Related Skills

- **e-scan** (eskill-quality): Run e-scan before this skill to include security verification steps in the deployment checklist.
- **e-deps** (eskill-coding): Run e-deps before this skill to ensure dependency health checks are part of the deploy process.
- **e-config** (eskill-quality): Run e-config before this skill to verify environment configurations are correct before deploying.
