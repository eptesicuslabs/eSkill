...
Use `fs_list` and `fs_read` to check for these files. If multiple ecosystems are present (e.g., a monorepo), audit each one separately.
...
Execute the appropriate audit command using `shell_exec` from the eMCP shell server:
...
- **Monorepo with shared dependencies**: Packages in a monorepo may pin different versions of the same dependency. Report version inconsistencies and recommend hoisting or aligning versions.
- **Transitive-only vulnerabilities**: A vulnerable package may only be reachable through a transitive dependency. Updating the direct dependency may not resolve it if the direct dep pins the vulnerable version.
- **Deprecated packages without vulnerabilities**: A package can be deprecated (unmaintained) without having known CVEs. Flag deprecated packages separately since they represent future risk even without current vulnerabilities.
- **Optional and peer dependencies**: Optional dependencies that fail to install do not break the build but may disable features. Peer dependency mismatches cause subtle runtime errors. Audit both categories.
- **Private registry packages**: Packages from private registries lack public vulnerability databases. Note that private packages cannot be audited for CVEs and recommend internal security review.

## Related Skills

- **e-scan** (eskill-quality): Run e-scan alongside this skill to cover both dependency and source code vulnerabilities.
- **e-license** (eskill-quality): Run e-license alongside this skill to verify dependency licenses comply with project requirements.
- **e-sbom** (eskill-quality): Follow up with e-sbom after this skill to produce a formal software bill of materials from the audit results.
