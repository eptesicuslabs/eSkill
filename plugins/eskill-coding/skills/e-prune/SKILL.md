...
- **Squash-merged branches**: Squash-merged branches do not appear as "merged" in `git branch --merged` because the squash commit has a different SHA. Detect squash merges by comparing branch tip diffs against main.
- **Branches used by CI/CD**: Some CI systems create and reference branches (e.g., `gh-pages`, `release/v*`). Detect CI-referenced branches from pipeline configs before suggesting deletion.
- **Shared branches with multiple contributors**: A stale branch may have commits from other developers who consider it active. Check the author list before recommending deletion.
- **Branches with associated open PRs**: A branch with an open pull request should not be deleted even if it appears stale. Cross-reference with the remote to detect open PRs.
- **Worktree-attached branches**: Branches checked out in other git worktrees cannot be deleted. Detect worktree attachments and report them rather than producing a failing delete command.
