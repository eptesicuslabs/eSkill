...
Use `fs_list`, `fs_read`, and `data_file_read` from the eMCP data-file server to read manifest files and detect the stack.
...
- If the target file contains no route handlers or component boundaries, report this to the user and suggest the e-testgen skill for unit tests instead.

## Edge Cases

- **Shared database state between tests**: Integration tests that do not isolate database state will pass individually but fail when run together. Ensure each test uses transactions or truncation to reset state.
- **External service dependencies**: Tests hitting real external APIs (payment gateways, email services) should be mocked at the HTTP layer. Detect external URLs in the codebase and inject stubs.
- **Authentication middleware**: Routes behind auth middleware need valid tokens or session cookies in test requests. Generate helper functions that create authenticated test clients.
- **File upload endpoints**: Multipart form data endpoints require special test request construction. Detect file upload handlers and generate tests with mock file payloads.
- **Rate limiting in tests**: If the application has rate limiting, rapid test execution may trigger limits. Detect rate limit middleware and either disable it in the test environment or add appropriate delays.

## Related Skills

- **e-testgen** (eskill-coding): Run e-testgen before this skill to ensure unit-level scaffolds exist before generating integration tests.
- **e-factory** (eskill-testing): Run e-factory alongside this skill to generate realistic test fixtures for integration scenarios.
- **e-e2e** (eskill-testing): Follow up with e-e2e after this skill to extend integration tests into full end-to-end workflows.
