...
Use `filesystem` tools (list_dir, read_text) to inspect manifest files and detect the ORM:
...
- **performance-analysis** (eskill-coding): Follow up with performance-analysis after this skill to assess the broader performance impact of detected N+1 queries.
- **database-workflow** (eskill-coding): Follow up with database-workflow after this skill to apply query optimizations and schema changes that fix N+1 patterns.
