...
- **Renamed symbol used in string-based lookups**: Frameworks that reference functions by string name (dependency injection containers, route registries, serialization) will break on rename. Search for string literals matching the old name.
- **Re-exports through barrel files**: Renaming a symbol that is re-exported through an index.ts barrel file requires updating the barrel, and any consumer importing from the barrel path.
- **Circular dependency introduction**: Moving a function between modules can create a circular dependency. Check the import graph before and after the move.
- **Module initialization order changes**: Moving code between modules can change when side effects execute (e.g., global registrations, static initializers). Verify that startup behavior is unchanged.
- **Cross-language references**: In polyglot projects, a Python service may reference a JavaScript module's export name in configuration. Refactoring the JavaScript side alone leaves the Python reference broken.

## Related Skills

- **e-testgen** (eskill-coding): Run e-testgen before this skill to establish a safety net for refactoring changes.
- **e-deadcode** (eskill-coding): Run e-deadcode before this skill to identify removal candidates as part of the refactoring plan.
- **e-review** (eskill-coding): Follow up with e-review after completing this skill to summarize refactoring changes for reviewers.
