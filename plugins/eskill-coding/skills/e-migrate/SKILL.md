...
- Read existing migration files using `fs_read` and `fs_list` to reconstruct the schema history.
...
- If a test database is available, execute against it using `shell_exec`.
...
- **Large table migrations**: Adding a column to a table with millions of rows can lock the table for minutes. Recommend online DDL techniques (pt-online-schema-change for MySQL, `CONCURRENTLY` for PostgreSQL indexes).
- **Renaming columns vs. adding new ones**: Column renames in most databases require creating a new column, copying data, and dropping the old one. A migration that uses `ALTER COLUMN RENAME` may not work across all databases.
- **Data migrations mixed with schema migrations**: Migrations that both alter schema and transform data are harder to roll back. Split into a schema-only migration followed by a data-only migration.
- **Foreign key constraints during migration**: Adding or removing foreign keys on populated tables can fail if referential integrity is violated. Validate data consistency before applying constraint changes.
- **Enum type modifications**: Adding values to an enum is usually safe, but removing or renaming enum values can break existing rows. Check for rows using the value before modifying the enum.

## Related Skills

- **e-deploy** (eskill-devops): Follow up with e-deploy after this skill to verify migration steps are included in the deploy plan.
- **e-review** (eskill-coding): Follow up with e-review after this skill to summarize schema and query changes for reviewers.
