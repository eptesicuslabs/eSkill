...
Use `log_parse` from the eMCP data-file server if the profiling output is structured. Otherwise, read the output using `fs_read` and parse manually.
...
- **Cold start vs. warm measurements**: The first request to a service incurs initialization costs (JIT compilation, connection pool creation, cache priming). Measure both cold and warm performance and report them separately.
- **Garbage collection pauses**: Languages with GC (Java, Go, Node.js) may show periodic latency spikes from collection cycles. Profile GC independently from application logic to distinguish the two.
- **Benchmark results that vary across runs**: Noisy benchmarks (variance >20%) indicate environmental interference. Pin CPU frequency, close competing processes, and run multiple iterations before drawing conclusions.
- **Premature optimization in prototype code**: If the code is still being iterated on, deep performance work may be wasted. Confirm that the code path is stable before investing in optimization.
- **Performance regression after dependency update**: A new library version may introduce a performance regression. Compare benchmarks before and after the update to isolate the dependency as the cause.

## Related Skills

- **e-nplus** (eskill-coding): Run e-nplus before this skill to identify database query bottlenecks contributing to performance issues.
- **e-bundle** (eskill-frontend): Run e-bundle alongside this skill to assess frontend asset sizes as part of the performance review.
