...
- **Very large diffs (500+ lines)**: For diffs exceeding 500 lines, summarize by file group and focus review on high-risk areas rather than line-by-line analysis. Flag the large diff size as a review concern itself.
- **Renamed or moved files**: Git may report a renamed file as deleted+created rather than renamed. Detect renames to provide meaningful diff analysis of the actual content changes.
- **Binary file changes**: Binary files (images, compiled assets) cannot be reviewed textually. Report their presence and size change, and recommend visual review for image assets.
- **Generated code in the diff**: Auto-generated files (protobuf, GraphQL codegen, lockfiles) inflate the diff. Identify generated files and summarize them separately from hand-written code changes.
- **Diff against non-main base branches**: When the target branch is not main/master (e.g., a release branch or another feature branch), use the correct merge base for the diff rather than defaulting to main.

## Related Skills

- **e-testgen** (eskill-coding): Run e-testgen before this skill to ensure new code has test coverage for reviewers to evaluate.
- **e-scan** (eskill-quality): Run e-scan before this skill to include vulnerability findings in the review summary.
- **e-lint** (eskill-quality): Run e-lint before this skill to catch style violations before the review begins.
