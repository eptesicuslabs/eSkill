...
Use `read_text` to attempt to read CHANGELOG.md from the project root. Also check for alternate names: CHANGELOG, CHANGES.md, HISTORY.md.
...
If creating a new changelog, write the following header using `write_text`:
...
Use `read_text` to load the full contents of CHANGELOG.md. Then use `markdown_headings` to extract the structural outline.
...
Use `write_text` to write the complete file. Do not use partial writes or appends -- always write the full file to avoid corruption.

After writing, use `read_text` to read the file back and confirm it was written correctly.
