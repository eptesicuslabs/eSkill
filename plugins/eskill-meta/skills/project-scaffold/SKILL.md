...
Create all directories using `create_directory`. Then create placeholder source files using `write_text` with minimal but functional content. Each source file should contain a basic hello-world or library stub appropriate to the purpose.

## Step 3: Generate Configuration Files

### Package Manifest

Use `write_text` to create the appropriate manifest:
...
Use `write_text` to create a LICENSE file. Default to MIT license. Replace the year with the current year and the copyright holder with "Eptesicus Laboratories" unless the user specifies otherwise.
...
3. Run `git_commit` with the message "Initial project scaffold" and description "Generated by eskill-meta project-scaffold"
...
1. Use `list_directory` to verify all expected files and directories exist
