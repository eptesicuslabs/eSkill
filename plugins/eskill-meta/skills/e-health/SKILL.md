...
- A project with a git repository, test suite, and dependency manifest (package.json, pyproject.toml, or equivalent).
- The eMCP filesystem, shell, data_file_read, and test_run servers available.
- Read access to the project's CI configuration and documentation directory.
...
Also consider generating a trend chart if historical data is available from previous health reports stored in `.e-health/`.
...
- **Greenfield projects with no test suite**: A project with zero tests scores 0% on the testing category. Report this without penalizing other categories, and recommend e-testgen as the first action.
- **Vendored dependencies**: Projects with committed vendor directories may report inflated dependency counts. Exclude vendor/ from dependency health checks and note the vendoring strategy.
- **Documentation in external systems**: Some projects store docs in Confluence, Notion, or a separate docs repo. If no docs are found in-repo, ask whether docs exist externally before scoring documentation health as zero.
- **Archived or maintenance-mode projects**: A project intentionally in maintenance mode may have outdated dependencies and stale branches by design. Allow the user to set a maintenance profile that adjusts scoring thresholds.
- **Monorepo with independent packages**: Each package may have different health characteristics. Score packages independently rather than aggregating the entire repo into a single dashboard.

## Related Skills

- **e-deploy** (eskill-devops): Follow up with e-deploy after this skill to address any health issues before the next deployment.
- **e-ship** (eskill-meta): Follow up with e-ship after this skill to evaluate whether health metrics meet release criteria.
