...
- An active coding session with at least one code change, decision, or investigation to summarize.
- The eMCP filesystem and shell servers available for reading git state and recent changes.
...
- Chains created within the session time window
- Chains referencing the current branch or feature
...
If no reasoning server is available or no decisions are recorded, check commit messages for decision context (messages often contain reasoning after a blank line in the body).
...
If the user wants to append to an existing session log, read the existing file first with `fs_read`, then write the combined content.
...
- **No git changes in session**: The user may have spent the session reading code, investigating, or planning without committing. Capture decisions and findings even when there are no diffs.
- **Multiple branches touched**: A session that switches between branches needs per-branch summaries to avoid mixing unrelated changes into a single recap.
- **Uncommitted work at session end**: Stashed or unstaged changes are easily lost between sessions. Flag uncommitted work prominently and recommend committing or stashing before ending.
- **Pair programming sessions**: When multiple people contribute changes, attribute work to the correct author using git log rather than assuming a single contributor.
- **Long sessions spanning multiple goals**: Sessions exceeding 4 hours often cover multiple unrelated goals. Group the recap by topic rather than chronologically.

## Related Skills

- **e-context** (eskill-intelligence): Follow up with e-context after this skill to package the session recap for use in future sessions.
- **e-retro** (eskill-meta): Follow up with e-retro after this skill to analyze patterns across multiple session recaps.
