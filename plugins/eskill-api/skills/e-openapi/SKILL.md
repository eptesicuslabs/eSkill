...
- This skill validates the specification document itself, not the running API. To verify that an implementation matches its spec, use the e-contract skill.
- For GraphQL schemas, use the e-graphql skill instead.
...
- To generate mock servers from a validated spec, use the e-mock skill.

## Related Skills

- **e-apidoc** (eskill-office): Run e-apidoc before this skill to produce the documentation that validation will check against.
- **e-contract** (eskill-api): Follow up with e-contract after this skill to generate tests that enforce the validated API contracts.
