...
- **No spec and no types**: If neither an API spec nor typed route handlers exist, inform the user that contract extraction requires at least one structured source. Suggest creating an OpenAPI spec first using the api-mock-generator skill's spec extraction capabilities.
...
- **GraphQL APIs**: This skill targets REST/HTTP APIs. For GraphQL contract testing, use the graphql-schema-review skill to validate the schema and generate operation-level tests.
...
- To validate an existing OpenAPI spec for correctness before generating contracts, use the openapi-validator skill.

## Related Skills

- **openapi-validator** (eskill-api): Run openapi-validator before this skill to confirm the API spec is valid before generating contract tests.
- **integration-test-generation** (eskill-coding): Follow up with integration-test-generation after this skill to extend contract tests into broader integration coverage.
