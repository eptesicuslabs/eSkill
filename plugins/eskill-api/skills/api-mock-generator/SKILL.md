...
- For validating the real API against its spec, use the openapi-validator skill.
- For generating contract tests that verify real API compliance, use the contract-test-generator skill.
...
- **openapi-validator** (eskill-api): Run openapi-validator before this skill to ensure the API spec used for mock generation is valid.
- **contract-test-generator** (eskill-api): Follow up with contract-test-generator after this skill to verify mocks match the actual API contracts.
