...
1. Use `filesystem` `list_dir` with recursive traversal to enumerate all files in the scope directories.
...
1. The manifest contains:
   - **Metadata section**:
     ...
     - Generator identifier: "eskill-quality/file-integrity".
   - **Files section**:
...
3. Use `filesystem` `write_text` or equivalent to write the JSON manifest.
