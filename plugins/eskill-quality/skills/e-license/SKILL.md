...
- **Dual-licensed dependencies**: Some packages offer a choice between licenses (e.g., MIT OR Apache-2.0). Evaluate the most permissive option that satisfies the project's compatibility requirements.
- **Transitive dependency license conflicts**: A direct dependency may be MIT-licensed, but one of its transitive dependencies may be GPL. Scan the full dependency tree, not just direct dependencies.
- **No license declared**: A dependency with no LICENSE file and no license field in its manifest is legally ambiguous (not public domain). Flag these as requiring investigation.
- **License exceptions and additional permissions**: Some licenses include SPDX exceptions (e.g., `GPL-2.0-only WITH Classpath-exception-2.0`) that modify compatibility. Parse the full SPDX expression.
- **Bundled vs. linked dependencies**: GPL compatibility depends on how the code is used. Dynamically linked libraries have different compliance requirements than statically bundled code.

## Related Skills

- **e-deps** (eskill-coding): Run e-deps alongside this skill to combine license and vulnerability analysis of project dependencies.
- **e-sbom** (eskill-quality): Follow up with e-sbom after this skill to include license data in the software bill of materials.
