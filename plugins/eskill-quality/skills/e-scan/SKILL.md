...
- **Framework-provided sanitization**: Modern frameworks (React, Angular, Rails) auto-escape output by default. A raw HTML string in React is only vulnerable if passed through `dangerouslySetInnerHTML`, not in JSX expressions. Reduce severity for framework-protected contexts.
- **SQL in ORM query builders**: ORMs like SQLAlchemy, ActiveRecord, or Prisma parameterize queries by default. Only flag SQL injection when raw SQL strings are constructed with string concatenation or interpolation.
- **Intentional crypto in non-security contexts**: MD5 used for cache keys or content fingerprinting is not a vulnerability. Only flag weak crypto algorithms when used for password hashing, token generation, or encryption.
- **Test files with hardcoded credentials**: Test fixtures may contain fake API keys or passwords for mock services. Verify that flagged credentials in test directories are not real before reporting as findings.
- **Server-side code in isomorphic apps**: Next.js, Nuxt, and similar frameworks mix server and client code. A SQL query in a server component is normal; the same query in a client-bundled file is a critical finding.

## Related Skills

- **e-review** (eskill-coding): Follow up with e-review after this skill to include security findings in the code review summary.
- **e-deps** (eskill-coding): Run e-deps alongside this skill to cover both source code and dependency vulnerabilities.
- **e-secrets** (eskill-quality): Follow up with e-secrets after this skill to rotate and remove any hardcoded secrets that were detected.
