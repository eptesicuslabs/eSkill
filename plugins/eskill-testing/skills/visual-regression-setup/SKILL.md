...
Identify how the UI is built to determine the capture strategy. Use `filesystem` tools (list_directory, read_file) to scan the project:
...
Use `shell` tools (run_command) from the eMCP shell server:
...
- **storybook-scaffold** (eskill-frontend): Run storybook-scaffold before this skill to create the component stories that visual regression tests will capture.
- **e2e-orchestration** (eskill-testing): Follow up with e2e-orchestration after this skill to include visual checks in end-to-end test workflows.
