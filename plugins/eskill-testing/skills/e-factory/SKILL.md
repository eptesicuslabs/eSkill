...
Identify where data models are defined. Use `filesystem` tools (fs_list) to scan for schema files, then use `ast_search` from the eMCP AST server to extract model structures:
...
- **e-integ** (eskill-coding): Follow up with e-integ after this skill to use the generated test data in integration test scenarios.
- **e-e2e** (eskill-testing): Follow up with e-e2e after this skill to supply test data to end-to-end test workflows.
