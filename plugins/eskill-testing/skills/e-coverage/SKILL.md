...
Use `shell` tools (shell_exec) if `test_run` does not support the coverage flags natively. Ensure the output format is machine-readable (JSON, lcov, or XML).
...
- **Coverage without assertions**: A line is "covered" if it is executed, even if no assertion validates its behavior. High coverage with weak assertions provides false confidence. Recommend the e-mutate skill for assertion quality analysis.
...
- **e-threshold** (eskill-coding): Run e-threshold before this skill to define the thresholds that the coverage report will evaluate against.
- **e-mutate** (eskill-testing): Follow up with e-mutate after this skill to validate that covered code is effectively tested.
