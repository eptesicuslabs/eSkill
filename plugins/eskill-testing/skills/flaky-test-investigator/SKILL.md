...
Use `filesystem` tools (read_file) to read CI configuration files and `log` tools (log_read) to access recent CI logs:
...
Run the test multiple times to confirm and characterize the flakiness. Use `test_run` from the eMCP test server or `shell` tools (run_command):
...
- **e2e-orchestration** (eskill-testing): Run e2e-orchestration before this skill to reproduce flaky test failures in a controlled environment.
- **test-coverage-reporter** (eskill-testing): Follow up with test-coverage-reporter after this skill to reassess coverage after fixing or removing flaky tests.
