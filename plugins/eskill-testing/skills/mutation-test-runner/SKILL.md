...
Identify the project's language and test runner by inspecting manifest files using `filesystem` tools (list_directory, read_file):
...
Check if the tool is already installed by looking for its config file or checking the dependency list. If not installed, present the install command and wait for user confirmation before running it via `shell` tools (run_command).
...
- **Flaky tests during mutation**: If tests are flaky, they may randomly kill or miss mutants. Recommend fixing flaky tests first (use flaky-test-investigator skill).
...
- **coverage-threshold-check** (eskill-coding): Run coverage-threshold-check before this skill to identify areas with low coverage where mutation testing is most valuable.
- **test-scaffolding** (eskill-coding): Follow up with test-scaffolding after this skill to generate tests for code areas where mutations survived.
