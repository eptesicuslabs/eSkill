...
- Shell access to run process inspection commands (ps, top, lsof, or OS-equivalent).
- The eMCP shell server available for executing system commands.
- Sufficient permissions to read process information (some process details require elevated privileges).
...
Use `sys_procs` to retrieve the complete list of running processes. For each process, collect:
...
For processes that are flagged as suspicious or consuming unexpected resources, gather additional details using platform-specific commands via `shell_exec` (shell):
...
Choose the appropriate commands based on the detected platform from `sys_info`. Run only the commands relevant to the flagged processes to avoid excessive overhead.
...
- **Zombie processes**: Zombies show up in process lists but consume no resources. They indicate a parent process not reaping child exits. Identify the parent process rather than trying to kill the zombie directly.
- **Processes running inside containers**: Containerized processes appear as host processes but with PID namespacing. Cross-reference with Docker/containerd process IDs to map container names to host PIDs.
- **Short-lived processes**: Rapidly spawning and exiting processes (e.g., PHP-FPM workers, shell script forks) may not appear in a single `ps` snapshot. Use process accounting or repeated sampling to catch them.
- **Kernel threads vs. user processes**: Kernel threads (kworker, ksoftirqd) appear in process lists but cannot be killed or configured. Distinguish kernel threads from user processes to avoid misleading recommendations.
- **macOS vs. Linux command differences**: `ps aux` output format differs between macOS and Linux. Memory reporting (RSS vs. VSZ) and process tree tools (`pstree` vs. `ps --forest`) vary by platform.

## Related Skills

- **e-ports** (eskill-system): Follow up with e-ports after this skill to resolve port conflicts detected during process analysis.
- **e-snapshot** (eskill-system): Run e-snapshot before this skill to capture baseline system state before analyzing processes.
