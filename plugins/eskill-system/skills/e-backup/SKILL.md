...
- Write access to a backup destination directory (local disk, network share, or mounted volume).
- The eMCP filesystem and shell servers available for file operations and checksum computation.
- Sufficient free disk space at the destination for the backup archive plus working room.
...
Use the filesystem tools (`fs_list`) to enumerate all files within the backup scope. Apply the inclusion and exclusion rules from Step 1.
...
2. **SQL dump**: Use `sqlite3 <database> .dump` via `shell_exec` to produce a SQL text dump. This is safer for active databases as SQLite handles locking internally during the dump. Save the dump as `<database-name>.sql` and include it in the archive.
...
- **Database backups requiring credentials**: Database dumps need connection credentials that may not be available in the current shell environment. Prompt for credentials or detect them from application config files rather than failing silently.
- **Files locked by running processes**: On Windows, files open by running applications cannot be copied. Detect locked files, report them, and recommend stopping the relevant service before retrying.
- **Backup destination on the same disk**: A backup to the same physical disk provides no protection against disk failure. Warn when source and destination share the same mount point or drive letter.
- **Large node_modules and build artifacts**: Including node_modules, .venv, or build/ in the backup dramatically increases archive size. Exclude dependency and build directories by default since they can be recreated from lockfiles.
- **Symlinks pointing outside the project**: Symlinks to files outside the project root may cause the backup to include unexpected data or break on restore. Resolve symlinks and document external references in the manifest.

## Related Skills

- **e-checksum** (eskill-quality): Follow up with e-checksum after this skill to verify backup checksums and confirm data integrity.
- **e-recover** (eskill-devops): Follow up with e-recover after this skill to document the backup procedures in the recovery plan.
