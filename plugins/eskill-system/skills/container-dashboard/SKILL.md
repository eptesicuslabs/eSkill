...
Before proceeding, verify that Docker is available on the system. Run `docker --version` via `run_command`. If Docker is not installed or the daemon is not running, report the issue clearly and stop. Do not attempt to proceed without a functioning Docker installation.
...
If any container is categorized as Critical, use `notify_send` to dispatch a desktop notification with the following content:
