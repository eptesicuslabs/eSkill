...
- Log files or log streams accessible from the current environment (file paths, Docker logs, or journal entries).
- The eMCP filesystem and shell servers available for reading and parsing log files.
- A time window or error pattern to focus the investigation (otherwise the skill scans all recent entries).
...
- **Structured JSON logs with nested fields**: Log entries in JSON format may have deeply nested error details (e.g., `error.cause.stack`). Parse the full structure rather than matching only top-level fields.
...
- **e-containers** (eskill-system): Run e-containers before this skill to identify which containers are producing error logs.
- **e-monitor** (eskill-devops): Follow up with e-monitor after this skill to add alerting rules for the log patterns discovered.
