...
Use the filesystem tool (`read_file`) to read each file that exists. Do not fail if a file is absent; simply skip it. Collect all discovered requirements into a structured list of `(tool, required_version, source_file)` tuples.

## Step 2: Check OS and Platform

Use `system_info` to retrieve:
...
For each required tool discovered in Step 1, run the appropriate version command via `run_command` (shell tool). Use the following commands:
...
Use `system_env` to read the current environment. Check for variables commonly required by projects:
...
Use `system_disk` to retrieve disk usage information. Evaluate:
